...
class SimModuleNode final : public ModuleNode {
 public:
  SimModuleNode(LoweredFunc func, 
                ...
                std::string dev_code, std::string top_code, 
                std::string host_code, std::string cfg_code, std::string platform, 
  ...
  PackedFunc GetFunction(
    ...
    return PackedFunc([this](TVMArgs args, TVMRetValue* rv){
        ...
        GenHostCode(args, shmids, arg_types, func_, 
                    platform_, host_, top_, arg_names_, empty, options_["project"]);
 ...
 private:
  LoweredFunc func_;
  std::string dev_, top_, host_, cfg_;
...
Module CreateSimModule(
    ...
    std::vector<std::string> arg_names,
    std::string dev_code, std::string top_code, 
    std::string host_code, std::string cfg_code, 
  ...
  std::shared_ptr<SimModuleNode> n =
    std::make_shared<SimModuleNode>(
            func, arg_names, dev_code, top_code,
            host_code, cfg_code, platform, options);
  return Module(n);
}
} // namespace runtime
...
runtime::Module BuildSimModule(Array<LoweredFunc> funcs,
  ...
  return runtime::CreateSimModule(
          funcs[0], cg_host.arg_names, 
          cg_dev.GetDevice(), NULL,
          cg_host.GetHost(), cg_host.GetConfig(), platform, options);
}
...
runtime::Module BuildCatapultCSimModule(Array<LoweredFunc> funcs,
  ...
  return runtime::CreateSimModule(
          funcs[0], cg_host.arg_names, 
          cg_dev.GetDevice(), cg_dev.GetTop(), 
          cg_host.GetHost(), cg_host.GetConfig(), platform, options);
}
